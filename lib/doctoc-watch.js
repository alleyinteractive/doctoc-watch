...
var rootCwd = process.cwd(); // Current Working Directory
...
function init() {
  ...
  gaze.on('all', function (event, filepath) {
    ...
    if (event === 'changed' && filepath.substring(rootCwd.length) === program.target.substring(1)) {
      if (program.output) {
 ...
 */
function generateMarkdown(file, cwd) {
  var noLink = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];
...
function updateReadme(tree) {
  ...
  tree.push(['<!-- END doctoc generated TOC please', 'keep comment here to allow auto update -->'].join(' '));

  var dirtyFile = file.replace(['**Table of Contents**  *generated with', '[DocToc](https://github.com/thlorenz/doctoc)*'].join(' '), program.targetHeader).replace(['<!-- END doctoc generated TOC please keep', 'comment here to allow auto update -->'].join(' '), tree.join('\n'));
